...
Agent Skill для архитектурного надзора над сгенерированным и существующим кодом. Скилл помогает агенту найти уже реализованное поведение, проверить зависимости и API, обнаружить вредную связанность и выбрать минимальную языково-идиоматичную конструкцию. Для межмодульных задач он строит укрупнённые архитектурные представления с привязкой узлов и связей к доказательствам из репозитория. Режим сравнения сопоставляет два снимка в одинаковом масштабе и разделяет наблюдаемое изменение структуры и регрессию относительно заявленного архитектурного ограничения.
...
Статус: версия 0.1 — исследовательский прототип. Методика, тесты и парный eval harness реализованы. Зафиксированный прогон тогдашнего набора из 18 кейсов на Codex gpt-5.6-sol не показал общего прироста: средняя парная дельта составила +0,04, arch выиграл 12 сравнений, baseline — 14, ещё 28 завершились ничьей. Четыре architecture-view кейса дали +0,67, а прежние 14 кейсов — −0,14; весь набор использовался для настройки, поэтому прогон не даёт независимой проверки. Текущий набор содержит 21 кейс; три новых сценария сравнения версий ещё не участвовали в поведенческом прогоне. Результат и ограничения описаны в [отчёте от 19 августа 2026 года](docs/benchmark-2026-08-19.md).
...
- архитектурную дельту между commit, tag, release или каталогами: компоненты, типизированные связи, границы, владение состоянием, runtime-сценарии, публичные контракты и соответствие заявленным ограничениям.
...
~~~text
Сравни release/1.4 и release/1.5 endpoint-to-endpoint на component-уровне.
Разреши refs до commit ID, используй одинаковый scope и zoom, сопоставь переименованные
компоненты и отдели масштаб структурного изменения от conformance regression.
~~~
...
Основной файл содержит короткий рабочий алгоритм. Справочники подгружаются по задаче: методика ревью, сравнение снимков, архитектурные представления, выбор дизайна, метрики, языковые инструменты, примеры и научные источники.
...
- [Сравнение архитектуры между версиями](skills/arch/references/architecture-comparison.md)
