...
6. Как сравнивать архитектуру двух версий, не подменяя изменение файлов деградацией системы?
...
### Архитектурное сравнение требует намерения и стабильного отображения

[Perry и Wolf](https://www.cs.unibo.it/~cianca/wwwpages/readings/perrywolf.pdf) описывают архитектуру через элементы, форму и обоснование. Они различают erosion как нарушение архитектуры и drift как изменение, нечувствительное к ней; последующие работы используют эти термины с частично пересекающимися значениями. Поэтому сравнительный отчёт должен явно определять термин и называть проверяемое ограничение.

[Software Reflexion Models](https://doi.org/10.1145/222132.222136) сопоставляют intended high-level model, извлечённую source model и явное отображение между ними. Подход различает совпадающие, расходящиеся и отсутствующие связи. [Обзор static architecture conformance](https://homepages.dcc.ufmg.br/~mtov/pub/2010_ieeesw.pdf) рассматривает отсутствующую разрешённую связь отдельно от нарушения.

[Метрика structural distance](https://doi.org/10.1109/METRICS.2005.35) сравнивает связность двух версий. Авторы отдельно отмечают влияние уровня абстракции и отображения сущностей. [Systematic mapping study архитектурной эрозии](https://doi.org/10.1002/smr.2423) объединяет структурные, violation, quality и evolution symptoms; работа не устанавливает универсальный скаляр качества.

Следствие: baseline и target разрешаются до неизменяемых снимков и моделируются с одинаковыми scope, viewpoint, zoom, типами связей и конфигурацией анализатора. Переименованные и перемещённые файлы сопоставляются по ответственности, контракту, владельцу и runtime-роли.

Результат содержит две независимые части: масштаб наблюдаемого изменения и статус соответствия заявленным ADR, dependency rules, contracts или исключениям. При отсутствии архитектурного намерения conformance получает статус not assessed. Node churn и relation churn допустимы только как показатели структурного различия по стабильным логическим идентификаторам, а не как процент деградации.

Официальные документы Git задают различие [endpoint и merge-base diff](https://git-scm.com/docs/git-diff), [разрешение revision](https://git-scm.com/docs/git-rev-parse) и [изолированные worktree](https://git-scm.com/docs/git-worktree). Эти операции фиксируют воспроизводимый контракт сравнения и не требуют переключать рабочее дерево пользователя.
...
Рабочий алгоритм состоит из семи проверяемых решений:

1. Установить scope, contracts и target versions.
2. Для сравнения версий разрешить baseline и target, зафиксировать range semantics и одинаковый масштаб.
3. Построить локальную карту callers, data ownership, tests и dependency direction.
4. Выбрать нужный архитектурный вид и масштаб или явно отказаться от диаграммы.
5. Пройти reuse ladder.
6. Для finding связать signal с harm mechanism.
7. Выбрать минимальную correction и проверку.
...
- aligned comparison уменьшает ложные выводы о деградации из file churn, rename и несопоставимых graph extraction settings;
...
Eval сравнивает arch с baseline на одинаковых prompts. Проведённые прогоны проверяют прежние гипотезы только на открытом синтетическом наборе; скрытый набор и независимая человеческая оценка отсутствуют. Три architecture-comparison кейса добавлены в текущий набор, но ещё не участвовали в поведенческом прогоне.
